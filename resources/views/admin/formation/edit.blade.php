            ...
            <div class="minimal-card-wrapper">
                <div class="card mb-4 mt-5 mx-4 mx-sm-0 position-relative">
                    ...
                    <div class="card-body p-sm-5 text-center">
                        ...
                        <h4 class="fs-13 fw-bold mb-2">Modifier une formation</h4>
                        ...
                        <form action="{{ route('formation.update', $formation->id) }}" method="post" class="w-100 mt-4 pt-2">
                            @csrf
                            <div class="form-floating mb-4">
                                 @error('titre')
                                    <div class="text-danger mb-2">{{ $message }}</div>
                                @enderror
                                <input type="text" class="form-control" name="titre" value="{{ old('titre', $formation->titre) }}" required>
                            ...
                            <div class="form-floating mb-4">
                                @error('description')
                                    <div class="text-danger mb-2">{{ $message }}</div>
                                @enderror
                                <textarea class="form-control" name="description" placeholder="" id="floatingTextarea2" style="height: 100px">{{ old('description', $formation->description) }}</textarea>
                            ...
                            <select class="form-select mb-4" aria-label="duree" name="duree" required value="{{ old('duree') }}">
                                 @error('duree')
                                    <div class="text-danger mb-2">{{ $message }}</div>
                                @enderror
                                <option value="1" {{ old('duree') == '1' ? 'selected' : '' }}>6 Mois</option>
                                <option value="2" {{ old('duree') == '2' ? 'selected' : '' }}>1 ans</option>
                                <option value="3" {{ old('duree') == '3' ? 'selected' : '' }}>2 ans</option>
                                <option value="4" {{ old('duree') == '4' ? 'selected' : '' }}>3 ans</option>
                            ...
                            <div class="mt-5">
                                <button type="submit" class="btn btn-lg btn-warning w-100">Modifier</button>
                            </div>
                        </form>
                    </div>
                </div>
